...
def genere_config():
    ...
    try:
        ...
        return
    protocoles_dict = {p['nom'].lower(): p for p in data.get('protocoles',[])} #récupération des protocoles sous forme de dico/get en cas d'oubli crée un dico vide
    ...
    for r in data['routeurs']: #pour chaque routeur
        ...
        lines.append("!")
        protocoles_a_activer= set() #création d'un set vide != liste pour ne pas avoir de doublons active_protocols
        for interface in r['interfaces']:
            ...
            lines.append( "exit")
            lines.append("!")
        for proto  in protocoles_a_activer: #config le protocole avec le set une fois par routeru
            if proto.lower() in protocoles_dict: #converti en minuscule lower
                ...
                if p.get('nom').upper() == 'RIP': #upper majuscule
                    ...
                    lines.append(" exit")
                lines.append("!")
        if 'bgp_neighbors' in r and r['bgp_neighbors']:#on vérifie si le routeur a des voisins bgp dans json
